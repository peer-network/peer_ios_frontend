...
public class ConfirmPasswordResetMutation: GraphQLMutation {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"mutation ConfirmPasswordReset($token: String!, $password: String!) { resetPassword(token: $token, password: $password) { __typename status RequestId ResponseCode ResponseMessage } }"#
    ))

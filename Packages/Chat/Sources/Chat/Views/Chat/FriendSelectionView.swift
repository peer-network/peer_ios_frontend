...
struct FriendSelectionView: View {
    ...
    init(
        ...
        onCreateGroupChat: @escaping (String, [String]) async -> Result<String, APIError>
    ) {
        self.viewModel = viewModel
        self.isGroupChat = isGroupChat
        self.onDone = onDone
        self.onCreateGroupChat = onCreateGroupChat
    }
    ...
    private func friendRow(for user: RowUser) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                .fill(isSelected(user) ? Color.accentColor.opacity(0.15) : Color(.systemGray6))
        )
